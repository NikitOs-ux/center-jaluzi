...
// отрисовка картыы с выбранными товарами
function renderMaterialCard(data){
    let mainImg = document.querySelector('.main-img');
    let miniImg = document.querySelector('.mini-img');
    mainImg.setAttribute('src', `/img/zebraMaterials/${data.srcMainImg}`);
    miniImg.setAttribute('src', `/img/zebraMaterials/mini/${data.srcSmallImg}`);
...
function addItems(response){
        ...
        el.setAttribute('data-roll', response[i].id);
         ...
         <div class="item-viewBlock">
             <div class="item-images">
                 <img src="/img/zebraMaterials/${response[i].srcMainImg}" alt="" class="mainImg">
                 <img src="/img/zebraMaterials/mini/${response[i].srcSmallImg}" alt="" class="miniImg">
             </div>
         <div class="item-text">
             <span class="text-title">${response[i].name}</span>
             <span class="text-price">от ${response[i].price} &#8381;</span>
             <span class="text-color">${response[i].color}</span>
    ...
    window.onload = ()=>{                                               //onload - событие которое возникает сразу после загрузки страницы
        ...
        xhr.open('GET', '/serverScript/getZebraMaterials.php');                // создание условий запроса <'метод запроса', 'адрес скрипта, получающий клиентов из базы'>
