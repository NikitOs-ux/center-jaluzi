...
function renderMaterialCard(data){
    ...
    mainImg.setAttribute('src', `/img/rollMaterials/${data.srcMainImg}`);
    miniImg.setAttribute('src', `/img/rollMaterials/mini/${data.srcSmallImg}`);
...
function addItems(response){
        ...
        el.setAttribute('data-roll', response[i].id);
         ...
         <div class="item-viewBlock">
             <div class="item-images">
                 <img src="/img/rollMaterials/${response[i].srcMainImg}" alt="" class="mainImg">
                 <img src="/img/rollMaterials/mini/${response[i].srcSmallImg}" alt="" class="miniImg">
             </div>
         <div class="item-text">
    ...
    window.onload = ()=>{                                               //onload - событие которое возникает сразу после загрузки страницы
        ...
        xhr.open('GET', '/serverScript/getRollMaterials.php');                // создание условий запроса <'метод запроса', 'адрес скрипта, получающий клиентов из базы'>
